...
extern struct gpio_desc *rs_pin;
extern struct gpio_desc *dc_pin;
...
{
	int ret = spi_write(spidev,&data, 1);
	if(ret < 0){
		pr_info("Spi send data fail \n");
		return ret;
	}
	return 0;
}
...
void LCD_Pin(LCD_Pin_t pin, LCD_State_t state)
{
	...
	case LCD_Pin_DC:
		if (state != LCD_State_Low) {
			gpio_set_pin(dc_pin, PIN_HIGH);
		} else {
			gpio_set_pin(dc_pin, PIN_LOW);
		}
		break;
	case LCD_Pin_RST:
		if (state != LCD_State_Low) {
			gpio_set_pin(rs_pin, PIN_HIGH);
		} else {
			gpio_set_pin(rs_pin, PIN_LOW);
		}
		break;
	default: break;
	}
